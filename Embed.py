...
class PositionalEmbedding(nn.Module):
    def __init__(self, d_model, max_len=5000):
        ...
        pe = torch.zeros(max_len, d_model).float() # 确保类型一致
        ...
        self.register_buffer('pe', pe) #  将位置编码矩阵pe注册为模型的缓冲区，以便在保存模型时保存其状态，但不会将其视为模型参数。
...
class TokenEmbedding(nn.Module):
    ...
    def forward(self, x):
        x = self.tokenConv(x.permute(0, 2, 1)).transpose(1, 2) # 卷积输入(batch_size, in_channels, seq_len)
        return x
...
class FixedEmbedding(nn.Module):
    def __init__(self, c_in, d_model):
        ...
        # 将张量w包装成一个可训练参数，并将其赋值给self.emb对象的权重（weight）属性。
        # 但在训练过程中不会更新它的值。
    # 位置编码矩阵是在初始化时计算并固定的，在前向传播过程中，位置编码不会因为输入序列的不同而改变。
    def forward(self, x):
...
class raw_PatchEmbedding(nn.Module):
    def __init__(self, patch_len, stride, padding, dropout):
        super(raw_PatchEmbedding, self).__init__()
